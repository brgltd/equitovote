...
contract Healthcheck is EquitoApp {
	...
	event MessageReceived(uint256 indexed sourceChainSelector, string text);
	...
	function _receiveMessageFromPeer(
		...
		(string memory text) = abi.decode(messageData, (string));
		emit MessageReceived(message.sourceChainSelector, text);
	}
}
